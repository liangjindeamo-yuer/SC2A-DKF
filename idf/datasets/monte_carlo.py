...
class MonteCarloDataset(Dataset) :
    def __init__(self, dataroot:str, patch_size:int, augmentation:bool,
                 preload:bool, parallel_preload:bool, test:bool, spp:int) :
        super(MonteCarloDataset, self).__init__()
    ...
    def getPathList(self) :
        ...
        noisyPathList = listdir(self.dataroot)
        cleanPathList = listdir(self.dataroot)
        
        # Create List Instance for Adding File Name
        noisyNameList = [imageName for imageName in noisyPathList if ".png" in imageName and int(imageName.split("_")[-1].split(".")[0].replace("spp", "")) == self.spp]
        cleanNameList = [imageName for imageName in cleanPathList if ".png" in imageName and "32768spp" in imageName]
        
        # Sort List Instance
        noisyNameList = natsorted(noisyNameList)
        cleanNameList = natsorted(cleanNameList)
        
        return (noisyNameList, self.dataroot), (cleanNameList, self.dataroot)
